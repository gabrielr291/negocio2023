...
def register(request):
    ...
    if request.method =='POST':
        ...
        if form.is_valid():
            ...
            mail_subject = 'Por favor activa tu cuenta en la tienda Online'
...
def activate(request,uidb64,token):
    ...
    if user is not None and default_token_generator.check_token(user,token):
        ...
        request.error(request,'la activacion no es valida')
...
def resetPassword(request):
    if request.method == 'POST':
        ...
        if password == confirm_password:
            ...
            user.save()
            messages.success(request,'El password se ha reseteado correctamente')
            return redirect('login')
        else:
...
def edit_profile(request):
    ...
    if request.method == 'POST':
        ...
        if user_form.is_valid() and profile_form.is_valid():
            user_form.save()
            profile_form.save()
            messages.success(request, 'su informacion fue guarada con exito')
    ...
    if request.method == 'POST':
        ...
        if new_password == confirm_password:
            ...
            if success:
                ...
                messages.success(request, 'Su contraseña se ha actualizado exitosamente')
                return redirect('change_password')
            else:
                messages.error(request,'por favor ingrese una contraseña valida')
                return redirect('change_password')
        else:
            messages.error(request, 'La contraseña no coincide con la confirmacion')
            return redirect('change_password')
